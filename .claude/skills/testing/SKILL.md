...
- **E2E tests** (`tests/*.spec.ts`, Playwright) should be **fat** — one `test()` walks through many related assertions to amortize browser startup. Use `test.step('label', async () => {...})` inside the test so failure reports identify which step broke. Do not split fat E2E tests just to get better failure attribution; `test.step` solves that.
  ...
  Put a new assertion in whichever of the seven it belongs to. Add an eighth file only when a genuinely new kind of interface appears — not per feature. Note that splitting for speed no longer pays: the browser tier runs one spec at a time (`workers: 1`), because every action against the game waits for a frame and two headless browsers drawing the shop halve each other's frame rate. Another file adds a browser boot without shortening the wall. If the wall clock starts to hurt, the thing to try is a higher `E2E_RENDER_FPS` — each action's wait is a frame long, so a faster cap shortens every one of them.
