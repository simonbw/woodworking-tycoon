...
- **E2E tests** (`tests/*.spec.ts`, Playwright) should be **fat** — one `test()` walks through many related assertions to amortize browser startup. Use `test.step('label', async () => {...})` inside the test so failure reports identify which step broke. Do not split fat E2E tests just to get better failure attribution; `test.step` solves that.
  ...
  There are deliberately only **six spec files**, one per kind of interface, each swapping fixtures between halves rather than paying for a fresh page:
  ...
  Put a new assertion in whichever of the six it belongs to. Add a seventh file only when a genuinely new kind of interface appears — not per feature. Note that splitting for speed no longer pays: the browser tier is CPU-saturated at six workers, so a seventh file adds a browser boot without shortening the wall.
