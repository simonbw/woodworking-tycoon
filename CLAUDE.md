...
3. **Diegetic UI**: The shop floor is the game's only screen — there are no tabs. The canvas runs full-bleed with the garage drawn as a building on its lot (grass, driveway, walls, and the garage-door opening: `views/EnvironmentView.ts`), and the remaining chrome floats over it as a HUD (`shell/hud/EngineHud.tsx`). Everything else is an object reached from it:
   ...
   - **The Orange Box store** (`src/shell/scenes/StoreSceneRoot.ts` and `src/shell/scenes/store-views/`, planogram in `src/game/store-layout.ts`, keys in `src/game/store-interact.ts`): a shopping trip swaps the canvas to the store's own walkable floor — aisles generated from the registries (lumber and sheet piles per SKU, full-size machine displays, a back-wall tool run), every product its own bay with E/F at the shelf (after grabbing a flatbed from the corral by the entrance), the register's receipt card as the way home, and the truck out front for walking out on a cart. The lumberyard is still a menu overlay
