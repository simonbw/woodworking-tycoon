...
2. **Game Loop**: Managed by `Ticker` component for regular game updates; the player's body moves continuously between ticks with WASD (see `docs/continuous-movement.md`) while `GameState` tracks only the cell underfoot. The clock is spend-to-advance (see `docs/time-and-days.md`): full pace while the player is working, a near-real-time creep while idle, stopped at night — and the day only turns over by driving home to sleep. The player has no speed controls; the pause menu stops everything, and the other overlays don't stop the world.
