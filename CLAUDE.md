...
- **Shop state** (`src/game/GameState.ts`): the serialization schema — money, materials, machines, the stand's stock, a `ProgressionState` slice. At runtime state lives in the entities and the pure helpers take the narrow slices they read (issue #230); `sim/projection.ts` reads the entity world out as a full `GameState` only where a snapshot is the product (the save file, the DOM's render signal, the driver's assertions, the dev hooks), plus cheap slice reads (`projectPerson`, `projectProgression`, `projectTutorialFacts`); `sim/save/fixture.ts` builds the world back from one.
...
- **The views** (`src/views/`): one view entity per simulated thing, drawing it on the canvas.
- **The shell** (`src/shell/`): everything a player touches — the keys (`dispatch/`), the scenes (`scenes/`: the bench dive, the walkable store, the trip theater), and the DOM chrome (`hud/`) rendered by two React roots over the canvas.
- **Autosave** (`src/sim/save/SaveManager.ts`, `src/shell/saveSlot.ts`): the shop saves itself as it runs — coalesced idle writes, flushed synchronously on `pagehide`. Because a save is always waiting, "New Game" confirms with a card on the workbench rather than a browser `confirm()`

### Key Systems

1. **State Management**: The entity world is the state; every mutation goes through a command and announces itself as a domain event (`src/config/CustomEvent.ts`), which the live indexes (`sim/singletons/ShopGrid.ts`, the bench dive's cached stage) and the DOM signal (`src/shell/ShellStore.ts`) listen for — derived state costs mutations, not questions. The DOM reads the world through `projectGameState` on that signal, never on a tick
2. **Game Loop**: The engine ticks the world and renders it; the player's body moves continuously with WASD (see the header of `src/game/player-motion.ts`) while the simulation tracks only the cell underfoot. The clock is spend-to-advance (see `src/game/time-flow.ts`): full pace while the player is working, a ramping fast-forward under the held wait key, a slow creep while idle (~5× real time), stopped at night — and the day only turns over by driving home to sleep. The player has no speed controls; the pause menu stops everything, and the other overlays don't stop the world.
3. **Diegetic UI**: The shop floor is the game's only screen — there are no tabs. The canvas runs full-bleed with the garage drawn as a building on its lot (grass, driveway, walls, and the garage-door opening: `views/EnvironmentView.ts`), and the remaining chrome floats over it as a HUD (`shell/hud/EngineHud.tsx`). Everything else is an object reached from it:
   - **Shop manual** (`ManualProvider`): the `?` reference binder, an overlay
   - **The tutorials** (`src/shell/hud/tutorial/`, tracks and goals in `src/game/tutorial.ts`): handwritten to-do cards in the HUD's left column, one per active track — the guided opening, plus the sweeping lesson that begins when the floor first gets dusty — each showing one goal at a time, its steps as checkboxes that tick as the shop's state satisfies them, with the thing the first unchecked box names outlined in the world or ringed in the chrome. Steps are predicates over `GameState`, not a script (see the header of `src/game/tutorial.ts`)
   - **Journal** (`JournalModal`): the skill tree, opened from the top bar
   - **The for-sale stand** (model in `src/game/stand.ts`, street system in `src/sim/systems/StreetSystem.ts`): a small table with a hand-written FOR SALE sign in the grass at the end of the driveway — the game's one selling channel. The player carries finished pieces down and sets them out (F) or takes them back (E) (`StandPrompt`); customers stroll the sidewalk line below the lot, stop at a stocked stand, and buy at fair value (`getSellValue`) — there is no pricing step. `GameState.stand` holds what's set out, `GameState.customers` the passersby. Every sale settles instantly and queues a `PayoutEvent`; `RewardFlightLayer` (`src/shell/hud/payout/`) flies the coins and star to the HUD readouts. Sales are the game's only money and reputation source: the first sale unlocks the store, and reputation gates the lumberyard's channels
   ...
   - **The Orange Box store** (`src/shell/scenes/StoreSceneRoot.ts` and `src/shell/scenes/store-views/`, planogram in `src/game/store-layout.ts`, keys in `src/game/store-interact.ts`): a shopping trip swaps the canvas to the store's own walkable floor — aisles generated from the registries (lumber and sheet piles per SKU, full-size machine displays, a back-wall tool run), every product its own bay with E/F at the shelf (after grabbing a flatbed from the corral by the entrance), the register's receipt card as the way home, and the truck out front for walking out on a cart. The lumberyard is still a menu overlay, and the old store overlay survives behind `?website` as the future website (issue #200)
   ...
   - **The bench view** (`src/shell/scenes/bench/`, engine in `src/game/bench-work/`): Tab at a bench dives into a zoomed work surface where the pointer is the hand — prying pallets apart, tool-first work on the piece where it lies, clamps-first glue-ups, and blueprint assembly, all committing through the commands in `sim/commands/` (the gesture decides _when_, the command decides _what_). The world keeps ticking while the view is open. The system doc is `docs/bench-work.md`; single-module detail lives in the `bench-work/` module headers (blueprints in `blueprint.ts`, bench groups in `bench-group.ts`, glue-ups in `glue-up.ts`, the tool-first offer in `tool-work.ts`)
   - Shop layout management happens on the floor itself: machines are physically picked up, carried, and set down by the player (see `src/sim/commands/machine-commands.ts`)
...
- **Machine Carrying** (`src/sim/commands/machine-commands.ts`): Bought machines ride home crated in the truck's bed and are lifted out at the tailgate; shop-built ones land crated beside the bench. Either way the player carries them into place — there is no separate layout editor
...
```
src/
├── engine-main.ts         # Boots the game and stands its entities up
├── core/                  # The engine: entities, tick/render, renderer, input, sound
├── sim/                   # The world
│   ├── entities/          # Player, machines, piles, crates, the truck, the stand…
│   ├── singletons/        # Wallet, Clock, Progression, Consumables, Broom…
│   ├── systems/           # The passes that tick them
│   ├── commands/          # The only way anything in the world changes
│   ├── save/              # Save file, migrations, and the fixture loader
│   └── projection.ts      # The world read out as shop state
├── game/                  # The pure game: registries and the rules over them
│   ├── machines/          # Machine type definitions
│   ├── bench-work/        # Hand work: blueprints, groups, glue-ups, coverage
│   ├── tools/             # Tool definitions and their operations
│   └── *.ts               # Materials, values, progression, tutorial, helpers
├── views/                 # One view entity per simulated thing, drawn on the canvas
│   ├── machine-sprites/   # Machine renderers
│   └── material-sprites/  # Material renderers
├── shell/                 # Everything the player touches
│   ├── dispatch/          # Keys and targeting
│   ├── scenes/            # The bench dive, the walkable store, the trip theater
│   ├── hud/               # The DOM chrome over the canvas
│   │   └── store-page/    # The old storefront overlay — the future website, behind ?website
│   ├── HudRoot.tsx        # Screen-anchored React root (renders on signals)
│   └── OverlayRoot.tsx    # World-pinned React root (rides the camera every frame)
├── components/            # Shared DOM widgets the HUD is built from
...
- Every change to the world goes through a command in `src/sim/commands/` — nothing else writes entity state
- New machines should be added to `src/game/machines/` with corresponding sprites in `src/views/machine-sprites/`; if the sprite ships as procedural `Graphics` rather than art, add a row to `docs/asset-backlog.md`
