...
- `GameState.dust: Record<CellKey, Record<Species, number>>` — sparse,
  ...
  are meant to emit pseudo-species — `colorBySpecies` already has their
  colors — but the species side is missing, so plywood/MDF currently
  emit nothing: issue #126.)
