...
/** `barlow-condensed-600.woff2`, or `caveat-variable.woff2` for a range. */
...
async function main() {
  ...
  const header = [
    ...
    ` * own origin. Hand-written faces (the Lumberjack logo) live in`,
    ` * fonts.css instead. See scripts/fetch-fonts.ts. */`,
  ].join("\n");
