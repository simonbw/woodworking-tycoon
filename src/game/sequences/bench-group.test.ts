 ...
 * half of that — what the bench view leans on — through the real actions:
 * the run is found from the floor layout, stock on either table is on the
 * one surface, and a glue-up whose strips straddle the seam commits after
 * the pieces are slid onto one table (gatherBenchPiecesAction), which is
 * what the view does when the last clamp goes tight.
 */
...
import { gatherBenchPiecesAction } from "../game-actions/operation-actions";
...
function shopWithJoinedTables(): ShopDriver {
  ...
  const strips = shop.machine(WORKBENCH).inputMaterials;
  assert.strictEqual(strips.length, 3, "fixture stages three strips");
  return shop.arrange((state) => ({
    ...state,
    machines: [
      ...state.machines.map((m) =>
        m.machineTypeId === WORKBENCH
          ? { ...m, inputMaterials: [], benchLayout: {} }
          : m,
      ),
      {
...
describe("tables pushed together are one bench", () => {
  ...
  it("keeps a lone table's own measure when nothing is pushed against it", () => {
    const shop = openShop(cuttingBoardShop).arrange((state) => ({
      ...state,
      machines: [...state.machines, table("worktable1x2", [5, 6])],
    }));
    const machines = getMachines(shop.shop.machines);
