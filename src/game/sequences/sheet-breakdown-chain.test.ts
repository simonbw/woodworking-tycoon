...
import { CROSSCUT_SLED_BLUEPRINT } from "../bench-work/blueprint";
import { GameState } from "../GameState";
import { MACHINE_TYPES } from "../Machine";
import { makeToolItem, materialMeetsInput } from "../material-helpers";
import { MaterialInstance, SheetGood } from "../Materials";
import { getSheetBuyPrice } from "../material-values";
import { makeSheet } from "../sheet-helpers";
import { sheetSize } from "../sheetStock";
...
function shopWithSawhorses(): ShopDriver {
  const shop = openShop(endGrainShop).arrange((state: GameState) => ({
    ...state,
    money: 500,
    progression: {
      ...state.progression,
      unlockedSkills: [...state.progression.unlockedSkills, "jigsAndFixtures"],
    },
  }));
  shop.putEverythingDown();
...
describe("sheet breakdown chain", () => {
  ...
  it("crosscuts to length once the sled is on, and not before", () => {
    ...
    shop.arrange((state) => ({
      ...
      player: {
        ...state.player,
        inventory: [...state.player.inventory, makeToolItem("crosscutSled")],
      },
    }));
  ...
  it("ties up two clamps for the straightedge while the cut runs", () => {
    const shop = fetchSheet(shopWithSawhorses(), "full");
    shop.arrange((state) => ({ ...state, clamps: 1 }));
    shop
