...
export class ShopDriver {
  ...
  /**
   * Select an operation on a machine by id. Only what the station
   * *offers* counts — the old driver's lookup went through
   * availableOperations, so a locked skill or an unmounted tool reads as
   * "does not offer", not as a mystery id (sequence tests assert on
  ...
  select(
    ...
    const operation = offered.find((candidate) => candidate.id === operationId);
    if (!operation) {
      throw new Error(
        `${machineTypeId} does not offer "${operationId}" — it offers ` +
          `[${offered.map((op) => op.id).join(", ")}]. A locked skill or an ` +
          `unmounted tool is the usual cause.`,
      );
    }
