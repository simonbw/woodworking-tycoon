...
import { MaterialInstance, Board } from "../../game/Materials";
...
function twoToneShop(learned: boolean): ShopDriver {
  ...
  return shop
    .arrange((game) => {
      shop.progression.skillPoints = learned ? 1 : 2;
      if (learned) {
        shop.progression.unlockedSkills.push("twoToneBoards");
      }
      ...
      for (const material of strips.slice(2)) {
        game.addEntity(
          new MaterialPileEntity(material, shop.player.position, 0),
        );
      }
      const workbench = shop.machine(WORKBENCH);
      workbench.state = { ...workbench.state, inputMaterials: [] };
    })
...
describe("two-tone chain", () => {
  ...
  it("pays back the point it cost", () => {
    ...
    // $48 board -> 48 XP -> level 3 -> +2 points, so the one still unspent
    // plus the two just earned makes three.
