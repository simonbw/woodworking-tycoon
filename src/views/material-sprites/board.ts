...
/**
 * A board stood on its long edge, seen from above: nothing but the
 * narrow edge face (thickness × length) — the wide contact shadow is
 * what says the board stands. The edge is a window onto the board's
 * own scans, placed by its face region, so tipping a board up shows
 * the same wood it showed lying down. Same milled-state language as
 * createBoardSprite: unjointed edges weather gray, jointing lifts the
 * veil.
 */
export function drawBoardOnEdge(
  ...
  // The upturned edge face — nothing but it: the wide shadow is what
  // says the board stands
 ...
 * `EdgeBandSprite`. Just the band, no shadow (drawBoardOnEdge draws
 * the standing board; this draws the surface being worked). Unjointed
 * it hides under the weathered veil; `finished` shows the grain the
 * plane brought up.
 */
